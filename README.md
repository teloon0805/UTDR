...
项目以公开数据、公开代码和可复现实验为主。鱼雷只作为水下目标感知的应用背景，当前研究聚焦于公开场景下的目标检测、识别、跟踪与环境鲁棒性，不涉及武器制导、攻击决策或作战实现。
...
| 阶段 | 周次 | 学习内容 | 阶段产出 |
| --- | ---: | --- | --- |
| 编程与工具 | 1–2 | Python、NumPy、Matplotlib、Git、虚拟环境 | 可复用的基础代码与实验日志 |
| 数学与深度学习 | 3–8 | 线性代数、概率、梯度、PyTorch、CNN、迁移学习 | MNIST/CIFAR分类器、训练模板 |
| 计算机视觉 | 9–12 | IoU、NMS、mAP、YOLO、自定义数据集 | 第一个水下图像检测基线 |
| 水下光学 | 13–16 | 吸收、散射、色偏、增强与域变化 | 原图/增强图/模型的对照报告 |
| 信号与水声 | 17–22 | 采样、滤波、FFT、STFT、水声传播、声呐、CFAR | 声信号分析和声呐检测基线 |
| 融合与选题 | 23–24 | 光学+声呐融合、ByteTrack、卡尔曼滤波 | 研究问题、数据、基线与实验计划 |
...
- [x] Python 列表、循环和平均值计算（`lesson1.py`）
- [x] NumPy 数组、`shape`、`reshape`、索引与切片（`lesson2.py`）
- [ ] Matplotlib 基础绘图与二维数组可视化
- [ ] Git 实验记录与项目规范
- [ ] PyTorch 分类器
- [ ] YOLO 水下目标检测基线
- [ ] 水声信号频谱与时频分析
...
underwater/
├── README.md                 # 项目说明与学习路线
├── lesson1.py                # Python：列表、循环与平均值
├── lesson2.py                # NumPy：数组与矩阵切片
├── lessons/                  # 按学习阶段整理的练习
├── notebooks/                # 可交互实验与可视化
├── datasets/                 # 数据说明、下载地址与划分规则
├── experiments/              # 实验配置、结果和误差分析
├── models/                   # 模型配置与权重说明
├── reports/                  # 周报、复现报告和阶段总结
└── requirements.txt          # Python依赖
```

## 第一阶段：基础学习计划

### 第1周：Python、NumPy 与 Matplotlib
...
优先寻找公开的前视声呐、侧扫声呐、声呐图像分类和水下声学目标识别数据。真实军事目标数据通常不公开，因此研究将使用公开民用目标、仿真数据或合规采集数据。
...
本项目用于公开的水下感知、信号处理和机器学习研究。涉及的目标检测、识别与跟踪方法仅用于学术学习和民用场景验证，不提供武器设计、制导、攻击决策或作战部署方案。
